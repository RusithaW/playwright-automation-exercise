...
```text
├── pages/
│   ├── components/       # Shared UI components (*Component.ts)
│   └── *.ts              # One Page Object per page (locators + actions combined)
├── api/
│   ├── endpoints/        # API route definitions (*Endpoints.ts)
│   └── services/         # API request/response handling (*Service.ts)
└── tests/                # Spec files (ui/*.spec.ts, api/*.api.spec.ts)
```

1. **Page Objects (`pages/*.ts`)**: Each page combines its locators and interaction methods in a single class.
2. **API Endpoints (`api/endpoints/*Endpoints.ts`)**: Defines API routes and base URLs.
3. **API Services (`api/services/*Service.ts`)**: Wraps API calls with typed request/response handling.
